...
--		monochrome_textcolor = nil, -- If set, some colorizing is applied using this color
		...
		taskbar_clock_position_and_size = "9,9.8;6,0.7",
		node_color = 0,
		table_border = 'yes',
...
function theme_class:get_button(area, prefix, code, text, tooltip)
	local formspec
	if code == "close_window" then
		formspec = 'image_button_exit['..area..';'..self[prefix.."_button"]..';'..code..';'.. minetest.colorize(self[prefix.."_textcolor"] or self.textcolor,minetest.formspec_escape(text))..']'
	else
		formspec = 'image_button['..area..';'..self[prefix.."_button"]..';'..code..';'.. minetest.colorize(self[prefix.."_textcolor"] or self.textcolor,minetest.formspec_escape(text))..']'
	end
	if tooltip then
