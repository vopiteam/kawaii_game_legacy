...
local function pep_on_place(itemstack, placer, pointed_thing, pep_node_name, pep)
	if placer and placer:is_player() and pointed_thing.type == "node" then
		local under = pointed_thing.under
		local node = minetest.get_node(under)
		local node_def = minetest.registered_nodes[node.name]
		if node_def and node_def.on_rightclick and not placer:get_player_control().sneak then
			return node_def.on_rightclick(under, node, placer, itemstack, pointed_thing) or itemstack
		end
		...
		minetest.set_node(pos, {name = pep_node_name, param2 = dir})
