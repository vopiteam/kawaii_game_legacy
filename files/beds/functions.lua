...
local function lay_down(player, pos, bed_pos, state, skip, sit)
	...
	if state ~= nil and not state and not sit then
		...
		if sit then
			if bed_pos ~= nil then
				if player_api.is_sitting(player) == false then
					player_api.sit_down(bed_pos, player, S("To get up, click on the bed you are sitting on!"), false)
				end
			end
			beds.player[name] = 2
...
function beds.on_rightclick(pos, player)
	if player_api.is_sitting(player) == false then
		on_rightclick(pos, player)
	else
		if player_api.sitting_on_this_object(pos, player) then 
			on_rightclick(pos, player)
		else
			player_api.show_sitting_object_message(player)
		end
	end	
end
