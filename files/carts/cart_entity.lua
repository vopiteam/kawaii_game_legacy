...
function cart_entity:on_rightclick(clicker)
	...
	if self.driver and player_name == self.driver and player_api.is_sitting(clicker) == true then
		self.driver = nil
	...
	elseif not self.driver and player_api.is_sitting(clicker) == false then
		...
		player_api.sit_down({x = 0, y = 0, z = 0}, clicker, carts.S("To get up, click on the cart you are sitting on!"), false)
...
function cart_entity:on_detach_child(child)
	if child and child:get_player_name() == self.driver and player_api.is_sitting(child) == true then
		self.driver = nil
		carts:manage_attachment(child, nil)
		player_api.stand_up(child)
	end
end
...
function cart_entity:on_punch(puncher, time_from_last_punch, tool_capabilities)
	...
	if puncher:get_player_control().sneak then
		...
		if self.driver then
			...
			if player_api.is_sitting(player) == true then
				if self.old_pos then
					self.object:set_pos(self.old_pos)
				end
				carts:manage_attachment(player, nil)
				player_api.stand_up(player)
			end
		end
...
local function rail_on_step(self, dtime)
	...
	if ctrl and ctrl.jump and player_api.is_sitting(player) == true then
		self.driver = nil
		carts:manage_attachment(player, nil)
		player_api.stand_up(player)
	end
