...
local S = translator and translator("homedecor_gastronomy") or intllib.make_gettext_pair()
...
local bottle_colors = {
	{ "brown", S("Kola bottle"), S("Four Kola bottles") },
	{ "green", S("Fanfa bottle"), S("Four Fanfa bottles") },
}
...
for _, b in ipairs(bottle_colors) do
	...
	homedecor.register("bottle_"..name, {
		...
		walkable = false,
		groups = {snappy=3, food = 1, wieldview = 2},
		sounds = default.node_sound_glass_defaults(),
...
minetest.register_craft( {
        ...
        recipe = {
			{ "vessels:glass_bottle", "dye:orange" }
        },
})
...
minetest.register_craft({
	output = "homedecor:coffee_maker",
	recipe = {
	    {"basic_materials:plastic_sheet", "bucket:bucket_river_water", "basic_materials:plastic_sheet"},
	    {"basic_materials:plastic_sheet", "default:glass", "basic_materials:plastic_sheet"},
	    {"basic_materials:plastic_sheet", "basic_materials:heating_element", "basic_materials:plastic_sheet"}
	},
})
